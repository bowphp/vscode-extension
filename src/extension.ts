...
import { TintinFormattingEditProvider } from './providers/TintinFormattingEditProvider';
...
export function activate(context: vscode.ExtensionContext) {
  ...
  let tintinFormatCfg = vscode.workspace.getConfiguration('tintin.format');
  if (tintinFormatCfg.get<boolean>('enable')) {
    context.subscriptions.push(vscode.languages.registerDocumentFormattingEditProvider(documentSelector, new TintinFormattingEditProvider));
    context.subscriptions.push(vscode.languages.registerDocumentRangeFormattingEditProvider(documentSelector, new TintinFormattingEditProvider));
  }

  // Set html indent
  const EMPTY_ELEMENTS: string[] = ['area', 'base', 'br', 'col', 'embed', 'hr', 'img', 'input', 'keygen', 'link', 'menuitem', 'meta', 'param', 'source', 'track', 'wbr'];
  vscode.languages.setLanguageConfiguration('tintin', {
    indentationRules: {
  ...
  let client = new LanguageClient('tintin', localize('tintinserver.name', 'TINTIN Language Server'), serverOptions, clientOptions);
